  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      if (t < 0.10) {
        ...
        const flickerT = t / 0.10
        let flickerAlpha = 0
        if (flickerT < 0.5) {
          flickerAlpha = flickerT < 0.3
            ? (flickerT / 0.3) * 0.25
            : ((0.5 - flickerT) / 0.2) * 0.25
        } else {
          flickerAlpha = flickerT < 0.8
            ? ((flickerT - 0.5) / 0.3) * 0.35
            : ((1.0 - flickerT) / 0.2) * 0.35
        }
        ctx.fillStyle = `rgba(200, 220, 255, ${flickerAlpha})`
        ctx.fillRect(0, 0, texWidth, texHeight)

      } else if (t < 0.35) {
        const collapseT = (t - 0.10) / 0.25
        ...
        const staticAlpha = 0.4 * (1 - collapseT)
        if (staticAlpha > 0.01) {
          const frameIdx = Math.floor(elapsed / 50) % 2
          ctx.imageSmoothingEnabled = false
        ...
        const holdT = (t - 0.35) / 0.15

        const lineH = Math.max(1, 3 - holdT * 1.5)
        ctx.fillStyle = 'rgba(200, 220, 255, 1.0)'
        ctx.fillRect(0, texHeight / 2 - lineH / 2, texWidth, lineH)

        ctx.globalAlpha = (1 - holdT * 0.5)
        ctx.drawImage(glowCanvas, 0, texHeight / 2 - glowH)
        ctx.globalAlpha = 1

        const flashT = Math.max(0, (holdT - 0.4) / 0.6)
        ...
        const settleT = (t - 0.90) / 0.10

        if (destReady) {
          ctx.drawImage(destCanvas, 0, 0)
        }

        const overShootAlpha = 0.06 * (1 - settleT)
        ctx.fillStyle = `rgba(200, 220, 255, ${overShootAlpha})`
        ctx.fillRect(0, 0, texWidth, texHeight)
      }
